...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Figure 1. Historical number of horses per province in Canada since 1906."}
...
knitr::kable(max_horses, caption = "Table 1. Range of the number of horses for each provinces from 1940-1972")
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap=paste0("Figure 2. Historical number of horses in ",params$province," since 1906.")}
province_plot <- horse_pop %>% 
  ...
  xlab("Year") +
  ylab("Number of horses") +
  ggtitle(paste0("Historical number of horses in ",params$province)) +
  facet_grid(~GEO) +
  scale_y_continuous(labels = scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
province_plot
